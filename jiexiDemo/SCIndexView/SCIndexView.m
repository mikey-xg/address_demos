...
- (void)configSubLayersAndSubviews{
    BOOL hasSearchLayer = [self.dataSource.firstObject isEqualToString:@"#"];
    NSUInteger deta = 0;
    if (hasSearchLayer) {
        // 这里原本是画的一个查找的图，为了项目 -> 我改成‘#’了
...
- (CAShapeLayer *)createSearchLayer{
    CGFloat radius = 15 / 4;
    CGFloat margin = 15 / 4;
    CGFloat start = radius * 2.5 + margin - 7;
    UIBezierPath *path = [UIBezierPath bezierPath];
    [path moveToPoint:CGPointMake(start+1, start)];
    [path addLineToPoint:CGPointMake(start+9, start)];
    [path moveToPoint:CGPointMake(start, start+4)];
    [path addLineToPoint:CGPointMake(start+8, start+4)];
    
    [path moveToPoint:CGPointMake(start+4, start-2)];
    [path addLineToPoint:CGPointMake(start+2, start+6)];
    
    [path moveToPoint:CGPointMake(start+4+3, start-2)];
    [path addLineToPoint:CGPointMake(start+2+3, start+6)];
    [path closePath];
    CAShapeLayer *layer = [CAShapeLayer layer];
    layer.fillColor = UIColor.clearColor.CGColor;
    layer.strokeColor = self.configuration.indexItemTextColor.CGColor;
    layer.contentsScale = [UIScreen mainScreen].scale;
    layer.lineWidth = 15 / 12;
